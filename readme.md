...
For developing mn.css itself config for [Biome](https://biomejs.dev/) is in place and `bunx serve .` works great as an alternative to just using the `file://` protocol in the browser - even though that is a perfect valid approach as well. Tools like Playwright have some potential for adding a level of testing rigor that many CSS libraries often lack. But, so far nothing is in place for visual regression testing.
...
Putting the capabilities or ethics of LLMs mostly aside, I can see the value in the avoidance of tedium or what some might call 'unnecessary complexity'. LLMs do seem to have potential in avoiding unnecessary complexity, and boilerplate, including css boilerplate, is often that! Still, there is a major difference between using tech to write gobs of boilerplate for you and building or choosing a better conceptual system which has almost no boilerplate. Assuming the later is possible, settling for the former feels foolish in that it ends up being much more fragile in addition to being a death sentence for proper understanding, and proper understanding of our design primitives is crucial for good UX in the long run.

Another benefit generative AI advocates talk about is "lowering the floor" - LLMs are hardly the only way to do this. Better, simpler, more approachable libraries do this too, and can do it without relying on third party pay-to-play services, crazy energy use, or unethical model and training. Additionally better libraries that can accomplish this WHILE promoting understanding rather than discouraging it are a Win-Win-Win type of scenario.
